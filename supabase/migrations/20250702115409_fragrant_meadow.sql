/*
  # Update collect_payment RPC function to support reference codes

  1. Function Changes
    - Add p_reference_code parameter to collect_payment function
    - Include reference_code in payment insertion

  2. Security
    - Maintain existing security checks and RLS enforcement
    - Validate user permissions for all operations

  3. Backward Compatibility
    - Reference code parameter is optional (defaults to NULL)
...
-- Updated function with reference code parameter
...
BEGIN
  ...
  IF p_status NOT IN ('paid', 'pending', 'overdue', 'refunded', 'cancelled') THEN
    RAISE EXCEPTION 'Invalid payment status: %', p_status;
  END IF;

  -- Validate payment method if provided
  IF p_method IS NOT NULL AND p_method NOT IN ('cash', 'check', 'bank_transfer', 'online', 'adjustment') THEN
...
GRANT EXECUTE ON FUNCTION public.collect_payment(
  uuid, uuid, numeric(10,2), timestamptz, timestamptz, text, text, text, text, uuid, numeric(10,2)
) TO authenticated;